        ...
        foreach ($classes as $v) {
            ...
            if (empty($coach)) {
                Db::table('motion_template_log')
                    ->insert(array('source' => 2, 'coach_id' => $coach['id'], 'status' => 0, 'error' => '无此教练coach', 'create_at' => time(), 'create_time' => date('Y-m-d H:i:s'), 'byid' => $v['id'], 'templateId' => $templateId, 'type' => 4));
                continue;
            }
            if (empty($member)) {
                Db::table('motion_template_log')
                    ->insert(array('source' => 2, 'coach_id' => $coach['id'], 'status' => 0, 'error' => '无此教练member', 'create_at' => time(), 'create_time' => date('Y-m-d H:i:s'), 'byid' => $v['id'], 'templateId' => $templateId, 'type' => 4));
                continue;
            }
            if (empty($memberInfo)) {
                Db::table('motion_template_log')
                    ->insert(array('source' => 2, 'coach_id' => $coach['id'], 'status' => 0, 'error' => '无此教练memberInfo', 'create_at' => time(), 'create_time' => date('Y-m-d H:i:s'), 'byid' => $v['id'], 'templateId' => $templateId, 'type' => 4));
                continue;
            }
            if (empty($fans)) {
                Db::table('motion_template_log')
                    ->insert(array('source' => 2, 'coach_id' => $coach['id'], 'status' => 0, 'error' => '无此教练fans', 'create_at' => time(), 'create_time' => date('Y-m-d H:i:s'), 'byid' => $v['id'], 'templateId' => $templateId, 'type' => 4));
                continue;
            }
            if (empty($openid)) {
                Db::table('motion_template_log')
                    ->insert(array('source' => 2, 'coach_id' => $coach['id'], 'status' => 0, 'error' => '无此教练openid', 'create_at' => time(), 'create_time' => date('Y-m-d H:i:s'), 'byid' => $v['id'], 'templateId' => $templateId, 'type' => 4));
                continue;
            }
            $data['class_id'] = $v['id'];
